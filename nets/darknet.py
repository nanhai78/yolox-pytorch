...
class Focus(nn.Module):
    ...
    def forward(self, x):
        patch_top_left = x[..., ::2, ::2]  # 在宽和高上以步长为2进行切片
...
class CSPDarknet(nn.Module):
    def __init__(self, dep_mul, wid_mul, out_features=("dark3", "dark4", "dark5"), depthwise=False, act="silu", ):
        ...
        #   初始的基本通道是64
        # -----------------------------------------------#
        base_channels = int(wid_mul * 64)  # 64 第一个输出channel
        base_depth = max(round(dep_mul * 3), 1)  # 3  bottleneck循环的次数
        ...
        #   在主干网络中，下采样操作在csp_layer前，csp结构不改变特征的分辨率和channel数
        # -----------------------------------------------#
        self.dark2 = nn.Sequential(
            Conv(base_channels, base_channels * 2, 3, 2, act=act),  # Baseconv，如果是nano版本这里就是一个深度可分离卷积
    ...
    def forward(self, x):
        ...
        x = self.dark5(x)
        outputs["dark5"] = x
        return {k: v for k, v in outputs.items() if k in self.out_features}  # 返回dark3,dark4,dark5

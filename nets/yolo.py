...
# width参数控制channel的大小,depth控制着Bottleneck的循环次数
class YOLOPAFPN(nn.Module):
    def __init__(self, depth=1.0, width=1.0, in_features=("dark3", "dark4", "dark5"), in_channels=[256, 512, 1024],
                 depthwise=False, act="silu"):
        super().__init__()
        Conv = DWConv if depthwise else BaseConv
        # backbone,分别输出输出20,20,1024    40,40,512    80,80,256
        self.backbone = CSPDarknet(depth, width, depthwise=depthwise, act=act)  # backbone网络
    ...
    def forward(self, input):
        ...
        [feat1, feat2, feat3] = [out_features[f] for f in self.in_features]  # 由浅层到深层
...
class YoloBody(nn.Module):
    def __init__(self, num_classes, phi):
        ...
        depthwise = True if phi == 'nano' else False  # 只在nano版本为true
